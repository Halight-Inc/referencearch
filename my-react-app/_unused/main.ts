...
import { ChatHistoryManager } from "./play/ChatHistoryManager.js";
...
export namespace Something {
	...
	export let startButton: boolean = false
	export let stopButton: boolean = false
	...
	let userThinkingIndicator: any = null;
	let assistantThinkingIndicator: any = null;
	let transcriptionReceived: any = false;
	...
	async function initAudio() {
		try {
			console.log("Requesting microphone access..."); /* statusElement.textContent = "Requesting microphone access..."; */
			console.log('status:',"connecting"); /* statusElement.className = "connecting"; */
			...
			console.log("Microphone ready. Click Start to begin."); /* statusElement.textContent = "Microphone ready. Click Start to begin."; */
			console.log('status:',"ready"); /* statusElement.className = "ready"; */
			startButton = false;
		} catch (error) {
			console.error("Error accessing microphone:", error);
			console.log("Error: " + error.message);/*statusElement.textContent = "Error: " + error.message;*/
			console.log('status:',"error"); /* statusElement.className = "error"; */
		}
	}
	...
	async function initializeSession() {
		...
		console.log("Initializing session..."); /* statusElement.textContent = "Initializing session..."; */
		...
		try {
			...
			console.log("Session initialized successfully"); /* statusElement.textContent = "Session initialized successfully"; */
		} catch (error) {
			console.error("Failed to initialize session:", error);
			console.log("Error initializing session"); /* statusElement.textContent = "Error initializing session"; */
			console.log('status:',"error"); /* statusElement.className = "error"; */
		}
	}
	...
	async function startStreaming() {
		...
		try {
			...
			console.log('status:',"recording"); /* statusElement.className = "recording"; */
	...
	function stopStreaming() {
		...
		startButton = false;
		stopButton = true;
		console.log("Processing..."); /* statusElement.textContent = "Processing..."; */
		console.log('status:',"processing"); /* statusElement.className = "processing"; */
	...
	socket.on('streamComplete', () => {
		...
		console.log("Ready"); /* statusElement.textContent = "Ready"; */
		console.log('status:',"ready"); /* statusElement.className = "ready"; */
	});

	// Handle connection status updates
	socket.on('connect', () => {
		console.log("Connected to server"); /* statusElement.textContent = "Connected to server"; */
		console.log('status:',"connected"); /* statusElement.className = "connected"; */
		sessionInitialized = false;
	});

	socket.on('disconnect', () => {
		console.log("Disconnected from server"); /* statusElement.textContent = "Disconnected from server"; */
		console.log('status:',"disconnected"); /* statusElement.className = "disconnected"; */
		startButton = true;
		stopButton = true;
		sessionInitialized = false;
		hideUserThinkingIndicator();
	...
	socket.on('error', (error) => {
		...
		console.log("Error: " + (error.message || JSON.stringify(error).substring(0, 100)));/*statusElement.textContent = "Error: " + (error.message || JSON.stringify(error).substring(0, 100));*/
		console.log('status:',"error"); /* statusElement.className = "error"; */
		hideUserThinkingIndicator();
		hideAssistantThinkingIndicator();
	});
