...
import { getScenario, getSimulationById } from '@/api';
...
export default function Results() {
  ...
  const { id } = useParams();
  const simulationId = id || undefined;

  const [isLoading, setIsLoading] = useState(true);
  const [simulation, setSimulation] = useState<SimulationAttributes | undefined>(undefined);
  ...
  useEffect(() => {
    const fetchSim = async () => {
      ...
      try {
        const result = await getSimulationById(simulationId, token);
  ...
  if (!simulation || !scenario) {
    return <div className="p-4">Simulation not found</div>;
  }
      ...
      <main className="flex-1 p-4 overflow-y-auto w-full">
        ...
        <div className="bg-neutral-100 rounded-lg p-4 mb-6">
          ...
          <div className="space-y-3">
            {simulation?.simulationResult?.competencyEvaluations.map((category) => {
              const isExpanded = isCategoryExpanded(category.competency);
              return (
                <div key={category.competency} className="bg-white rounded-lg p-4 shadow-sm">
                  <button 
                    className="flex justify-between items-center w-full mb-3"
                    onClick={() => toggleCategory(category.competency)}
                  >
                    <h3 className="font-medium">{category.competency}</h3>
                    {isExpanded ? (
