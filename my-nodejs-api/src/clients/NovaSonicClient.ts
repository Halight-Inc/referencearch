...
export class NovaSonicClass implements IAIAgent {
  constructor() {
    const bedrockClient = new NovaSonicBidirectionalStreamClient(CONFIG);

    inactiveSessionChecker(bedrockClient);
    ioHandler(bedrockClient);
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async runPrompt(_systemContext: string, _prompt: string, _sessionId: string): Promise<string> {
...
const DefaultTextConfiguration = {mediaType: 'text/plain' as TextMediaType};
...
const DefaultSystemPrompt = 'You are a friend. The user and you will engage in a spoken ' +
...
class NovaSonicBidirectionalStreamClient {
  ...
  sessionLastActivity: Map<string, number> = new Map();
  sessionCleanupInProgress = new Set<string>();
  ...
  createStreamSession(sessionId: string = randomUUID(), config?: NovaSonicBidirectionalStreamClientConfig): StreamSession {
  ...
  async processToolUse(toolName: string, toolUseContent: object): Promise<any> {
  ...
  async parseToolUseContentForWeather(toolUseContent: any): Promise<{latitude: number; longitude: number;} | null> {
    try {
  ...
  async initiateSession(sessionId: string): Promise<void> {
    ...
    try {
      ...
      console.log('processResponseStream', response);
  ...
  dispatchEventForSession(sessionId: string, eventType: string, data: any): void {
    const session = this.activeSessions.get(sessionId);
    if(!session) {
      return;
    }
  ...
  createSessionAsyncIterable(sessionId: string): AsyncIterable<InvokeModelWithBidirectionalStreamInput> {
  ...
  async processResponseStream(sessionId: string, response: any): Promise<void> {
    const session = this.activeSessions.get(sessionId);
    console.log('processResponseStream', sessionId, session)
    if(!session) {
      return;
    }
        ...
        if(event.chunk?.bytes) {
          try {
            ...
            try {
              ...
              console.log(jsonResponse.event?.contentStart, jsonResponse.event?.audioOutput, jsonResponse.event?.toolUse);
              if(jsonResponse.event?.contentStart) {
                ...
                // function calling
                /* eslint-disable */
                const toolResult = await this.processToolUse(session.toolName, session.toolUseContent as any);
  ...
  setupSystemPromptEvent(sessionId: string,
  ...
  async sendToolResult(sessionId: string, toolUseId: string, result: any): Promise<void> {
    const session = this.activeSessions.get(sessionId);
...
class StreamSession {
  ...
  maxQueueSize = 200; // Maximum number of audio chunks to queue
...
function inactiveSessionChecker(bedrockClient: NovaSonicBidirectionalStreamClient) {
